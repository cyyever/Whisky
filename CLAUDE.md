...
- **Status: patch series + tooling COMMITTED to main (`f3578ac8`/`0f111132`/`256fcf5e`); Proton itself NOT the shipped stack.** Proton is **live-swapped** into `~/Library/Application Support/com.isaacmarovitz.Whisky/Libraries/Wine` (Whisky Wine backed up at `…/Libraries/Wine.whisky-bak`), DXMT built on top; reports `wine-11.0`, boots and runs (`cmd` works) after the msync crash fix. **Steam** boots through bootstrap + client + CEF webhelper (after patches `0014`–`0016`) but the client self-exits ~35s in before reaching a CM. Proven NOT an msync bug (patch `0017` uniform shadows eliminated the wineserver polling storm + all lost-wakeups, Steam still asserts at same timing) and NOT network/VPN/winsock — most consistent with **Rosetta x86-emulation perf in the steam.exe↔steamwebhelper CEF handshake** (webhelper watchdog fires before the emulated handshake finishes). See Steam-on-Proton note + docs/proton-migration.md. Canonical stack is still Wine 11.13 above — this is a parallel track
- Source `vendor/proton-wine/` is **gitignored** (like `vendor/wine`; tag `proton-wine-11.0-…`). Tracked in main: `patches/proton-wine/` (17-patch series) + `scripts/install-proton.sh`; `scripts/build-dxmt.sh` + `scripts/build-wine-x86.sh` gained env-var / source-reset hooks. No app wiring / plist / submodule pin yet
- `patches/proton-wine/` — disjoint file ownership, all pass `git apply --check`: **`0001`–`0007` build/portability** (de-linux ntdll futex/TEB; winedmo FFmpeg8 PCM BSF; win32u opengl w/o libEGL; media-converter pthread; amd_ags libdrm guard; loader64 wine64; rundll32 WS_VISIBLE). **`0008`–`0013` macOS capability ports**: `0008` msync fast-sync (~52 files, CrossOver port + the crash fix), `0009` winemac DXMT support, `0010` kernelbase IFEO Debugger, `0011` `WINE_NX_COMPAT` env, `0012` coreaudio hide virtual devices, `0013` server fsync de-linux. Roughly `0007`≈wine `0001`, `0009`≈wine `0002`+`0005`, `0010`≈wine `0003`, `0011`≈wine `0004`, `0012`≈wine `0007`. **`0014`–`0016` Steam-runtime sync/deadlock fixes** (all found live under Steam): `0014` msync mixed-wait hybrid (`msync_wait_mixed_any` polls a bounded `server_wait` on the server subset — msync can't natively wait on msync+server object sets; deadlocked RpcSs/MSI/cold-boot/`reg add`), `0015` combase rpcss cold-start race (`WaitNamedPipe` on `\\.\pipe\lrpc\irpcss` before returning; steam.exe raced ahead of rpcss → uncaught `RPC_S_SERVER_UNAVAILABLE` crash), `0016` ntdll FLS-callback-no-lock (drop `fls_section` around FLS destructor callbacks — **upstream Wine bug**: callback-under-lock inversion deadlocks Steam's bootstrap→client handoff; upstream only guards the process-*exit* variant). Build `0014` into unix `ntdll.so`; `0015`/`0016` into PE `combase.dll`/`ntdll.dll` **both arches** (steam.exe is 32-bit). **`0017` msync uniform inproc_sync shadows (architectural)**: `server/object.c default_get_sync()` lazily gives EVERY waitable object an msync-backed shadow (new `struct object.sync`), so no object stays "server-only" → server-only Process waits dropped 3201→~6, mixed-wait stalls 6+→0. Also makes `do_msync()` respect `WINEMSYNC` (was hardcoded on) + boot-crash dispatch guards (`NtResetEvent`/`NtPulseEvent`/`NtWaitForSingleObject`) + `mach_msg2`→libSystem `mach_msg` Rosetta wrapper. A genuine keeper but NOT the Steam fix
...
- `scripts/install-proton.sh` — installs `vendor/proton-wine/build` into a self-contained `Wine/` dir (default `INSTALL_DIR=vendor/proton-wine/dist`) mirroring `build-wine-x86.sh`: bundles x86_64 dylibs + ffmpeg, KosmicKrisp loader swap (Khronos loader as BOTH `libMoltenVK.dylib` and `libvulkan.1.dylib` + ICD manifest), rpath-patches `x86_64-unix/*.so`, sets wine.inf `Graphics=mac`, symlinks `wine64→wine`. Swap into Whisky manually with `cp -R`
