...
- **Uncommitted changes** (on disk, WhiskyKit compiles; `make app` NOT yet run/verified in UI): Proton locked as default backend — `BottleSettings.wineBackend` default+decode → `.proton`; `WhiskyWineInstaller.protonBinFolder` falls back to `Libraries/Wine` when `Libraries/WineProton` absent; ConfigView backend selector removed. `WINEESYNC=0` now scoped to the Proton backend (kept `=1` for legacy Whisky-Wine). CLAUDE.md trimmed of bug narrative. → next: `make app` + smoke-test a bottle launches on Proton, then commit.
- **SF4 (Ultra Street Fighter IV, appid 45760) black screen — investigation paused.** DXVK d3d9.dll (32-bit) is correctly auto-dropped next to `SSFIV.exe`; d3dx9_43 present. Root blocker this session: **Steam was logged off** (`[Logged Off, 0, 0]`), so `-applaunch 45760` never started the game — could not repro the render failure. → next: relog Steam (Follow System Proxy OFF), launch SF4 from the **Steam Play button**, capture `DXVK_LOG_LEVEL=info DXVK_LOG_PATH=<dir>` to see which render stage fails.
