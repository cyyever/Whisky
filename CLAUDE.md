...
- Status: **DXVK renders 仙剑5's full 3D scene** (sky, water, lit terrain) on Apple M2, **`VK_ERROR_DEVICE_LOST` eliminated** (121→0) by patches `0003`+`0004`. Remaining: a pink/rose color tint (render-correctness, not device loss) and the game exiting after the opening scene; launch is also flaky (stale package-file handles → `CPackageFile INVALID_HANDLE_VALUE` after rapid relaunch). The Metal-validation "8 MB buffer destroyed while in use" assertion is validation-only strictness — it does **not** cause device loss in a normal run. The **Gcenx/dxvk-macOS** fork (DXVK 1.10.3) *fails* device creation on this MoltenVK 1.4.1 (queries timeline_semaphore as an extension; MoltenVK exposes it as core) — so upstream v3.0 + these patches is the working path. No `build-dxvk.sh` yet — patches archived, not auto-applied.
