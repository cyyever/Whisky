...
- **Fixed since:** the `server/event.c req_event_op`/`query_event` `EXC_BAD_ACCESS` on `event->sync->ops` (msync object cast as `struct event`) — both handlers now branch on `obj.ops==&msync_ops` and read state via `msync_get_event_state()`/`msync_is_manual_event()` (committed `f41ab4b8`, in `0008`). The unified event-driven mixed-wait bridge's `get_sync` fix (subscribe the bridge to `obj->ops->get_sync(obj)`, not the handle object — server-backed objects wake through a separate sync obj so `wake_up` fired there, missing 100% of bridge signals) makes full msync boot + log in via the bridge (committed `f106896a`; `WINEMSYNC_UNIFIED` still opt-in). **Remaining:** the full-msync spin, localized to ANONYMOUS auto-reset events (see lever note below) — `NO_ANON_AUTOEVENT` works around it
- msync bisection levers (env, read by `dlls/ntdll/unix/msync.c`): `WINEMSYNC_NO_EVENT` (all events→server-sync), `NO_AUTOEVENT` (auto-reset events→server), and the finer **`NO_ANON_AUTOEVENT`** / `NO_NAMED_AUTOEVENT` (split auto-events by whether they're named vs anonymous). The confirmed-good levers (`NO_SEMAPHORE`/`NO_MUTEX`/`NO_MANUALEVENT`) were removed — semaphore/mutex/manual-event are always on msync now. **Localized (2026-07-24): the full-msync spin is caused by ANONYMOUS auto-reset events** — a Steam threadpool worker busy-polls (`WaitForMultipleObjects(timeout=0)` ~20k/s) a set of anon "work-available" events that stall under fast msync delivery (the known fsync-100%-CPU class). **`WINEMSYNC_NO_ANON_AUTOEVENT=1` (anon auto→server, NAMED auto + all else on msync) → Steam logs in + settles to normal CPU** — a finer, higher-coverage config than NO_AUTOEVENT; candidate to become the Wine.swift default. Server-side `WINE_EVT_TRACE` (msync_set_event/reset_event) + client `WINE_MRING`/`WINE_BRIDGE_TRACE`/`MSYNCSPIN` are the env-gated diagnostics that localized it
