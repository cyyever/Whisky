...
- **Open blocker:** FULL msync (events on msync) crashes wineserver in `server/event.c req_event_op` — `EXC_BAD_ACCESS` dereferencing `event->sync->ops` (a msync object cast as `struct event`; same class as the boot-crash already fixed in `0008`; diagnosed via live lldb — Rosetta cores show only host ARM64 frames). Bisected: SEMAPHORE + MUTEX + QUEUE on msync are safe, enabling EVENT is what crashes → events stay on server-sync (the `WINEMSYNC_NO_EVENT` lever) for now; next step is a guard at the `req_event_op` seam
- msync bisection levers (env, read by `dlls/ntdll/unix/msync.c`): `WINEMSYNC_NO_SEMAPHORE` / `NO_MUTEX` / `NO_EVENT` / `NO_AUTOEVENT` / `NO_MANUALEVENT` each route that object type to server-sync
