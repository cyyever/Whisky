...
## Xbox controller support (verified 2026-07-20, tests/controller/)
- **Bluetooth pads work end-to-end** (model 1708+ / Series pair directly with macOS; VID 045E PID 02FD, standard HID): macOS BT-HID → winebus SDL backend (SDL 2.32.10 x86_64, dlopened `libSDL2-2.0.0.dylib`) → winexinput → XInput slot 0. One device, no SDL/IOHID double-report.
- **The one required fix**: `SDL_JOYSTICK_MFI=0` in the Wine env (set in both `constructWineEnvironment` and `constructWineServerEnvironment` — winedevice.exe hosting winebus inherits from wineserver). On macOS 27.0 beta the GameController framework enumerates pads but **delivers zero input** (same regression shape as 26.0, fixed then in 26.1); the hint forces SDL onto its HIDAPI/IOKit path, which works and keeps rumble. Revisit at GA. winebus only reads env at startup — `wineserver -k` after changing it.
- **Wired USB**: macOS 15 Sequoia+ ships a native GIP driver for official pads (untested here — beware charge-only cables: pad LED stays dark). **Xbox Wireless Adapter dongle (045E:02FE)**: no macOS driver exists, pairing can't complete; Linux xow/xone are the open-source references if a port (arm64 libusb helper + thin winebus socket backend) is ever wanted.
- Layer probes live in `tests/controller/` (SDL enumeration/input natively, XInput/winmm inside Wine) — see its README for the debug ladder.
