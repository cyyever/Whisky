...
- Patches `0003`+`0004` cut the per-draw device loss (was ~121/frame), but a **residual intermittent `VK_ERROR_DEVICE_LOST` remained** (~half of launches) — an ~8 MB host-visible buffer destroyed while a just-completed command buffer still referenced it (`notifyExternalReferencesNonZeroOnDealloc` / `command buffer references deallocated object`). (The earlier "DXVK buffer-lifetime race" guess was **wrong** — DXVK's tracking is correct.)

### Device-loss root cause = a MoltenVK bug (FOUND, FIXED, PR'd — verified)
- Tracked down via a parallel code audit + a deterministic Metal-validation repro (`METAL_DEVICE_WRAPPER_TYPE=1`). On **macOS 15+ MoltenVK uses a residency set**, and `MVKQueueCommandBufferSubmission::getActiveMTLCommandBuffer()` (`MVKQueue.mm`) then creates the submission `MTLCommandBuffer` with `retainedReferences = NO` *and* disables live-tracking. **Residency ≠ lifetime**: `MVKBuffer::detachMemory()` `release`s the `MTLBuffer` the instant the app destroys it, but the non-retained command buffer keeps an *external* reference until Metal deallocs it — which is **after** its completion handler signals the submission fence. A spec-correct app (DXVK) that destroys a resource right after `vkWaitSemaphores` then frees it while the command buffer still references it → Metal abort → `VK_ERROR_DEVICE_LOST`. Key dead-end ruled out: **`vkResetCommandPool` even with `RELEASE_RESOURCES` does NOT release it** (the MTLCommandBuffer belongs to the *submission*, not the pool) → no DXVK-side deterministic fix exists; it had to be fixed in MoltenVK.
- **Fix** = retain references on the residency-set path (`getActiveMTLCommandBuffer`: add `if (_device->hasResidencySet()) needsRetain = true;`), matching the present path which already does this. → **[KhronosGroup/MoltenVK#2762](https://github.com/KhronosGroup/MoltenVK/pull/2762)**. Verified: a MoltenVK rebuilt with the fix gives **0 lifetime assertions under Metal validation and 0 `VK_ERROR_DEVICE_LOST`**.
- **Patched MoltenVK is installed**: `…/Wine/lib/libMoltenVK.dylib` (x86_64, ad-hoc signed) is the fixed build; original backed up as `libMoltenVK.dylib.orig` (revert by restoring it). Rebuild recipe: clone KhronosGroup/MoltenVK `v1.4.1`, apply the one `getActiveMTLCommandBuffer` hunk, `./fetchDependencies --macos && make macos`, `lipo -thin x86_64 Package/Release/MoltenVK/dynamic/dylib/macOS/libMoltenVK.dylib`, `codesign -s -`. (Bundled MoltenVK normally comes from x86 brew `molten-vk` 1.4.1; `Wine/lib` files are read-only — `chmod u+w` before replacing.)
- **Two upstream DXVK bugs also PR'd** (found in the same audit, real regardless of macOS): **[doitsujin/dxvk#5750](https://github.com/doitsujin/dxvk/pull/5750)** — `importBufferResource`/`importImageResource` self-assign `m_resourceCookie` (should be `allocationInfo.resourceCookie`); **[doitsujin/dxvk#5751](https://github.com/doitsujin/dxvk/pull/5751)** — `DxvkResourceAllocation::decRef` used `memory_order_acquire`; **landed in DXVK master** (maintainer committed `8dc0423070` "Fix memory ordering for release" as `release` and closed the PR — finding validated upstream). `0003`/`0004` are MoltenVK-specific, not upstreamed.

### OPEN — next to investigate after reboot: black window, NOT device-loss
- With device-loss fixed, both D3D9 games still show a **black window even though DXVK fully sets up rendering** — logs show `DXVK: v3.0`, `Found device: Apple M2 (MoltenVK)`, `D3D9DeviceEx::ResetSwapChain`, `Presenter: Actual swapchain … 1280×832, 3 images`, **0 `VK_ERROR_DEVICE_LOST`, 0 `Failed to create Direct3D9`** — and the game script runs (loads title/scene images). So DXVK renders + presents to the swapchain but the content never reaches the visible window: a **DXVK → MoltenVK → winemac Vulkan-surface presentation** problem. 仙剑5 *did* render its scene in some runs (~1.5 MB screenshots); **Trap Yuri Garden is always black** — diff the two (window mode, present mode `IMMEDIATE` vs `FIFO`, how winemac attaches the `CAMetalLayer` for the Vulkan swapchain surface vs the DXMT path in patches `0003`/`0004`). Debug tools that worked: Metal API validation (`METAL_DEVICE_WRAPPER_TYPE=1`); DXVK stderr (grep with `-a`, logs are Shift-JIS-tainted); `WINEDEBUG=+relay` (firehose, ~1000× slowdown, but confirms the call path). Vulkan validation layers do **not** insert under wine's winevulkan (it loads MoltenVK directly, bypassing the loader/layers).
