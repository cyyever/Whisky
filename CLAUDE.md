...
- Wiring: `Wine/lib/libMoltenVK.dylib` is **actually the x86_64 Khronos Vulkan loader** (winevulkan dlopens that leaf name; the loader exports `vkGetInstanceProcAddr`, the Mesa ICD doesn't); ICD manifest at `~/.local/share/vulkan/icd.d/kosmickrisp_icd.x86_64.json`. **`build-wine-x86.sh` owns the swap** (fires only when `vendor/kosmickrisp/` exists). No MoltenVK backup kept — recoverable from the x86 brew `molten-vk` keg
