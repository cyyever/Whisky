...
- `patches/dxvk/` makes the MoltenVK-missing device features optional (`geometryShader`, `shaderCullDistance`, `depthClipEnable`, `robustBufferAccess2`, `nullDescriptor`) and forces `primitiveRestartEnable` on for strip/fan topologies (Metal can't disable primitive restart). Run with `MVK_CONFIG_API_VERSION_TO_ADVERTISE=4206592` (advertise Vulkan 1.3) + `WINEDLLOVERRIDES=d3d9=n` + the DXVK `d3d9.dll` in the game dir.
- Status: **WIP** — DXVK now creates the D3D9 device on Apple GPU (was impossible), but rendering is not yet reliable (intermittent `VK_ERROR_DEVICE_LOST`). The game also hits a separate `wine d3dcompiler:assemble_shader` failure (a Wine limitation, not DXVK). The **Gcenx/dxvk-macOS** fork has these MoltenVK fixes battle-tested and is the likely path to completion. No `build-dxvk.sh` yet — patches are archived, not auto-applied.
