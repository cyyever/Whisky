...
- **Steam-on-Proton gotchas**: (1) turn **Follow System Proxy OFF** — the geph HTTP proxy breaks Steam's CM (WSS→403); CMs are directly reachable. (2) msync-only (`WINEESYNC=0`) — macOS has no eventfd for esync. **OPEN CONFLICT:** `BottleSettings.swift` (`enhancedSync == .msync`) deliberately sets `WINEESYNC=1` alongside `WINEMSYNC=1` — a lie for DXMT (`lid3dshared.dylib` detects ESYNC and mis-behaves without it), which contradicts the Proton `WINEESYNC=0` requirement. Intended fix (decision pending, NOT implemented): scope `WINEESYNC=0` to the Proton backend while keeping the DXMT lie for Whisky-Wine. (3) `PROTON_DISABLE_LSTEAMCLIENT=1` (wired into `Wine.swift`) fixes the Proton lsteamclient tier0 crash ("reinstall Steam" box). (4) NOT a network/VPN/winsock issue (host reaches Steam HTTP direct; only raw CM ports GFW-blocked). CLI launch needs `whisky steam-fix <bottle>` first
...
- CEF GPU / ES3 (details: docs/steam-webhelper-ifeo.md §1): the shipped wrapper appends only `--no-sandbox --in-process-gpu` → ANGLE Renderer11 on **wined3d** (not DXMT), capped at **GLES 2.0** but renders correctly. **ES3 route (CONFIRMED, flicker bug OPEN, UNCOMMITTED):** add `--use-angle=vulkan --use-cmd-decoder=passthrough` → ANGLE-Vulkan → winevulkan → KosmicKrisp → Metal gives GLES 3.0 (the `GLES 3.0 > 2.0` error is gone), but the UI flickers (lead: KosmicKrisp WSI IMMEDIATE-vs-FIFO present mode; force-FIFO experiment in progress). Forcing wined3d SM4/FL_10 on Apple GL is a **DEAD END** (patch `0016` committed `53b99179`, reverted `d0994f4b` — makes ANGLE Renderer11 hang; Apple GL 4.1 lacks `EXT_shader_integer_mix`). DXMT is for games, not the UI
- Testing lesson: always launch Steam with the **full bottle env** (`WhiskyCmd shellenv <bottle>` dumps it) — a hand-rolled minimal env missing `WINEDLLOVERRIDES=…winemetal=b` + `DYLD_FALLBACK_LIBRARY_PATH` makes `steam.exe` spin ~100% with no login window; the app's `constructWineEnvironment` boots fine (~1.4%). Kill cleanly with `pkill -9 -f steamwebhelper` (+ `cef.win64`), not just `steam.exe`
